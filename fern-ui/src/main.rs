...
mod views;
/// Define an api module that contains server functions for our app.
mod api;
...
#[cfg(feature = "server")]
type AppStateExtension = dioxus::server::axum::Extension<server::AppState>;
...
fn main() {
    ...
    dioxus::serve(|| async move {
        ...
        let server = fern_server::Server::builder().await.start();
        let app_state = server::AppState::from(server);

        let router = dioxus::server::router(App).layer(dioxus::server::axum::Extension(app_state));
        Ok(router)
    });
}
